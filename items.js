...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        this.db.collection('item').aggregate(aggregate).toArray(function(err, categories) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        "use strict";
        let match = { category: category };
        if (category === 'All') {
            match = {};
        }
        const sort = { _id: 1 };
        const skip = page * itemsPerPage;
        this.db.collection('item').find(match).sort(sort).skip(skip).limit(itemsPerPage)
    ...
    this.getNumItems = function(category, callback) {
        ...
        this.db.collection('item').aggregate(aggregate)
            .toArray(function(err, totalInCategory) {
                // console.log(totalInCategory);
                assert.equal(null, err);
                callback(totalInCategory[0].num);
            });
        /*
         * TODO-lab1C:
         *
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        this.db.collection('item').aggregate(aggregate)
            .toArray(function(err, totalInSearch) {
                // console.log(totalInSearch);
                assert.equal(null, err);
                callback(totalInSearch[0].num);
            });
        // var numItems = 0;
    ...
    this.getItem = function(itemId, callback) {
        "use strict";
        const match =  { _id: itemId };
        this.db.collection('item').find(match).limit(1).next( (err, item) => {
            assert.equal(null, err);
            // console.log(item);
            callback(item);
        });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        const match =  { _id: itemId };
        const update = { $push: { reviews: reviewDoc } };
        this.db.collection('item').update(match, update, (err, res) => {
            assert.equal(null, err);
