...
function CartDAO(database) {
    ...
    this.getCart = function (userId, callback) {
        ...
        this.db.collection('cart').find(match).limit(1).next((err, cart) => {
    ...
    this.itemInCart = function (userId, itemId, callback) {
        "use strict";
        const match = {userId: userId, 'items._id': itemId};
        const projection = {"items.$": 1};
        this.db.collection('cart').find(match, projection).limit(1).next((err, cart) => {
            assert.equal(null, err);
            //console.log(cart.items[0]);

            callback(cart === null ? cart : cart.items[0]);
        });
        /*
         *
         * TODO-lab6
    ...
    this.updateQuantity = function (userId, itemId, quantity, callback) {
        "use strict";
        const match = {userId: userId, 'items._id': itemId};
        let projection = {$set: {"items.$.quantity": quantity}};
        if (quantity === 0) projection = {"$pull": {"items": { _id: itemId}}};
        const updateProperties = {upsert: false, returnOriginal: false};
        this.db.collection('cart').findOneAndUpdate(match, projection, updateProperties, function (err, res) {
            assert.equal(null, err);
            console.log(res);
            callback(res.value);
        });
